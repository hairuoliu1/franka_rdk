...
def record_loop(
    ...
    while timestamp < control_time_s:
        ...
        elapsed_s = int(timestamp)
        if control_time_s is not None and elapsed_s != last_logged_elapsed_s:
            remaining_s = max(control_time_s - timestamp, 0.0)
            if episode_idx is None:
                print(
                    f"LOOP_PROGRESS phase={phase_label} elapsed_s={timestamp:.1f} remaining_s={remaining_s:.1f}",
                    flush=True,
                )
            else:
                progress = (
    ...
    try:
        ...
        with VideoEncodingManager(dataset):
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                ...
                if listener is None:
                    logging.warning(
                        "Keyboard listener unavailable (headless mode). Continuing without right-arrow start gate."
                    )
                else:
                    logging.info(
                    ...
                    while not events["exit_early"] and not events["stop_recording"]:
                        time.sleep(0.05)
                    # Consume the start signal and avoid an immediate early-exit in record_loop.
                    events["exit_early"] = False
                    # Ignore accidental rerecord key presses before recording has started.
