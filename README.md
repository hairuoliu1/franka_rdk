...
└── lerobot/                             # LeRobot 框架 (git submodule)
    └── src/lerobot/
        ├── franka_rdk/                  # 注册、相机预设、ROS bridge
        ├── robots/                      # Franka 机器人封装
        ├── teleoperators/               # GELLO 遥操作封装
        └── policies/openpi_client/      # OpenPI 客户端策略
```
...
ROS 2 Humble 默认使用 Python 3.10。当前数据采集链路也按 Python 3.10 运行：主进程加载 LeRobot，`lerobot.ros.ros_bridge` 会用 `/usr/bin/python3.10` 作为 ROS bridge 子进程读取 ROS 2 话题。
...
```bash
cd /home/franka/franka_rdk/ros2/src/franka_gello_state_publisher/scripts/
...
```bash
cd /home/franka/franka_rdk/ros2
colcon build
...
```bash
LEROBOT_RUN_FRANKA_TESTS=1 \
/home/franka/miniconda3/envs/lerobot/bin/python \
  lerobot/tests/datasets/test_franka_data.py gripper-test
```
...
```bash
# 编译 (首次)
cd /home/franka/franka_rdk/ros2/src
...
正式数据采集直接使用 `lerobot.scripts.lerobot_record`，推荐用模块方式启动：

```bash
cd /home/franka/franka_rdk
/home/franka/miniconda3/envs/lerobot/bin/python -m lerobot.scripts.lerobot_record ...
```

采集脚本使用双臂封装：

- `lerobot.robots.bi_franka_fr3_robotiq_gripper.BiFrankaFr3RobotiqGripper`：读取 left/right Franka state 和相机。
- `lerobot.teleoperators.bi_gello_leader.BiGelloRosLeader`：被动监听 left/right GELLO action，不向机器人重复下发指令。
- `lerobot.ros.ros_bridge`：由 Python 3.10 子进程读取 ROS 2 话题，并写入 `/dev/shm/lerobot_state_<namespace>.json`，避免 Python 3.12 环境直接 import `rclpy`。

`lerobot` 中的单臂 `franka_fr3_robotiq_gripper` 和 `gello_ros_leader` 默认使用左臂 namespace `left`。双臂封装里左臂沿用这个默认值，右臂配置显式覆盖为 `right`。
...
默认输出路径为 `$HF_LEROBOT_HOME/<repo_id>`；使用 `--dataset.root` 可指定本地目录。`lerobot.scripts.lerobot_record` 是多 episode 交互式录制流程，按右方向键开始每个 episode，按 Esc 停止整个录制流程。
...
当前采集只使用三路相机：中间是 ZED，左右两侧是 OpenCV。默认配置写在 `configs/camera.yaml`，相机 key 必须全局唯一。录制脚本 `scripts/record_bimanual.sh` 直接复用这里的默认相机布局。
...
- `lerobot.scripts.lerobot_record`：正式录制入口

Franka 相关调试能力收敛到 `lerobot/tests/datasets/test_franka_data.py`，支持以下子命令：

| 子命令 | 用途 |
|------|------|
| `action-listener` | 监听 GELLO action |
| `fake-action` | 模拟发送 action |
| `obs-listener` | 监听 robot observation |
| `fake-state` | 模拟发送 observation |
| `camera-test` | 测试相机读取 |
| `gripper-test` | 测试夹爪读写 |
| `read-ee-pose` | 读取末端位姿 |
| `franka-ik` | Franka 逆解测试 |

```bash
LEROBOT_RUN_FRANKA_TESTS=1 \
/home/franka/miniconda3/envs/lerobot/bin/python \
  lerobot/tests/datasets/test_franka_data.py fake-action --namespace left

LEROBOT_RUN_FRANKA_TESTS=1 \
/home/franka/miniconda3/envs/lerobot/bin/python \
  lerobot/tests/datasets/test_franka_data.py action-listener

LEROBOT_RUN_FRANKA_TESTS=1 \
/home/franka/miniconda3/envs/lerobot/bin/python \
  lerobot/tests/datasets/test_franka_data.py fake-state --namespace left

LEROBOT_RUN_FRANKA_TESTS=1 \
/home/franka/miniconda3/envs/lerobot/bin/python \
  lerobot/tests/datasets/test_franka_data.py obs-listener
```

如果需要，也可以继续直接运行这个测试模块里的其他子命令。
